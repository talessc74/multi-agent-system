...
**Design-coherence escalation: console hero cards lacked Boardroom's glow signature (2026-06-18, ARGUS convocation of Galera de Design and UX).** Product owner judged the retextured console still read as visually disconnected from the redesigned Boardroom Home, despite both using `.glass`/`.glass-static` — not a token bug this time, but a missing design decision. The mandate given was explicit: the design team must look at Boardroom's now-approved visual identity and extend it into the console coherently and inventively, rather than receive another isolated patch instruction. CANVAS identified the specific visual gap: Boardroom's dossier card carries a per-mode-color radial glow (`.preview-wash`, restored earlier this milestone) and a `20px` border radius, signifiers entirely absent from the console's desktop hero input cards, which were flat-edged `.glass-static` boxes with no color identity of their own. FORGE's resolution: reuse the proven `.preview-wash` radial-glow technique rather than invent a new pattern — extracted into a shared `GlowWash` component (`App.tsx`, module scope) parameterized by `colorRgb`, applied with matching `border-radius: 20px` to every desktop input-step hero card: Modo 1/2's textarea card (mode-color glow), Modo 3's and Modo 4's Petição/Contestação card pairs (white glow on the neutral "Autor" card, mode-color glow on the "Réu"/respondent card, matching each card's existing left-border accent color), and Modo 5's Relato/Sentença-ou-Proposta/Anexar Documentos triple-card sequence (white glow on the first, mode-color glow on the other two, again matching existing accents). QUILL/COMPASS note: this intentionally reuses Boardroom's exact glow grammar rather than introducing a second visual idiom, so the two screens read as the same family rather than two designs that happen to share a blur effect. Scope was deliberately bounded to desktop input-step hero cards — the screens the product owner specifically flagged — not the confirm/result/sidebar surfaces already converted under this milestone's earlier passes; extending the glow there is left as a candidate follow-up, not silently included. `npm run build` and `npm test` clean (135 passed, 6 skipped, 0 regressions) with the flag both off and on.

**Sizing regression found and fixed on first staging pass (2026-06-18, same thread):** product owner flagged the deployed result as visually broken — the hero textarea's glow filled the entire card as a flat color wash instead of a corner accent, and the Boardroom sidebar showed an overwhelming, undampened purple blob. Root cause #1: `GlowWash`'s `radial-gradient(circle at 85% 0%, …, transparent 60%)` has no explicit size, so it defaults to `farthest-corner` sizing — on Boardroom's compact, near-square dossier card this reads as a tasteful corner highlight, but the console's hero cards (`min-h-[400px]`, 2-3x taller) have a much longer corner-to-corner distance, so the same percentage stop expanded the glow across the whole card. FORGE: a technique cannot be ported verbatim between containers of different proportions without checking how its sizing keyword behaves at the new scale — fixed by switching to a fixed pixel radius (`circle 280px at 100% 0%, …, transparent 70%`), which keeps the same corner-accent size regardless of container height. Root cause #2 (pre-existing, not introduced by the glow pass but exposed by the side-by-side comparison): the console's right "Boardroom" sidebar (`App.tsx` ~line 3915, `.glass-static`) sits directly behind the mesh's `b2` blob (520px, `#a23bff`, anchored top:30%/right:-140px of the viewport) with only a thin 10-16% white fill between them, so the blob shows through at near-full saturation over dense status text — Boardroom Home's dossier panel sits in the same blob position but has a busier, mode-colored layout that absorbs it better. First fix attempt: a one-off dark scrim added only to this sidebar panel.

**Second staging pass found the same bleed on a different panel (2026-06-18, same thread) — systemic fix instead of a third one-off patch:** product owner's next screenshot showed the hero textarea card (Modo 1/2) washed in solid teal — not `GlowWash` (already fixed above), but the `b3` mesh blob (`#00e0c7`, anchored bottom:-140px/left:30%) bleeding through that card's `.glass-static` fill the same way `b2` bled through the sidebar. Recognizing this as the same root cause appearing on a second panel — not two unrelated bugs — SCOUT/FLUX flagged the one-off sidebar scrim as exactly the kind of per-instance patching this milestone's own standing practice (see the "Deep audit closing the loop" risk above) warns against. Fixed structurally instead: the console's already-pinned `--glass-fill`/`--glass-fill-2` (pinned dark-on-purpose since the earlier contrast fix) were re-tinted from white-translucent (`rgba(255,255,255,0.1/0.16)`) to dark-translucent (`rgba(8,9,18,0.55)` / `rgba(10,12,22,0.65)`), so every `.glass-static`/`.glass` surface in the console mutes any mesh blob behind it to an ambient tint by construction, not per-panel. Border and specular tokens stay white for the frosted-edge highlight; only the body fill changed. The one-off sidebar scrim was removed as redundant. Scoped entirely to the console's inline pin — Boardroom Home's shared `--glass-fill` default is untouched. `npm run build` and `npm test` clean (135 passed, 6 skipped, 0 regressions).

**Light/dark toggle reported missing on console; scope decision on real light mode (2026-06-18, same thread).** Product owner asked why the console has no visible light/dark toggle button. Investigation found the button exists (`Navbar.tsx`, reused by the console) but the console's root pin (above) only covered `--glass-*`/`--mesh-*`, not `--text-primary`/`--text-secondary`/`--border`/`--bg-*` — those followed the global `[data-theme]` tokens unpinned. Toggling the global theme to light therefore left the console's background pinned dark while text/icon colors (e.g. the toggle's own icon, `color: var(--text-secondary)`) switched to their light-theme dark-navy values, rendering the button and "Meus Casos" control invisible (dark-on-dark) — confirmed by reading `index.css`'s `[data-theme='light']` block.

ARGUS escalated the broader question to the product owner via two explicit decisions rather than assuming scope: (1) hide the button vs. build real light mode vs. leave as-is — owner chose **build real light mode**; (2) given the discovery that the entire Mode Workspace console has *never* supported light mode (a pre-existing "tactical console" identity predating Liquid Glass, per Milestone 3's own scope note), proceed now vs. defer to its own milestone — owner chose **proceed now**.

SCOUT/FLUX assessed the actual surface: ~418 `text-white`/`border-white`/`bg-white`/-family occurrences across the console's render tree (`App.tsx`, ~2,270 lines from the console root to EOF), most authored directly against the fixed dark identity, some intentionally literal white-on-solid-color (chips/badges, hover-inverted buttons) that must NOT become theme-aware. Converting all of this safely needs a dedicated token strategy (a Tailwind `ink` color backed by an RGB CSS variable, mirroring the existing `--glass-fill` pattern, so opacity modifiers like `text-ink/70` keep working) and a careful per-section pass distinguishing "body chrome" from "intentionally-literal-white" — too large to complete safely in the same pass as the acute bug above without materially raising regression risk across the whole console.

**Decision:** fixed the acute, reported bug now — extended the existing console pin to cover the full token set (`--bg-primary/-secondary/-card`, `--text-primary/-secondary/-muted`, `--border`, `--border-active`) in addition to `--glass-*`/`--mesh-*`, so the console's background and text always move together regardless of the global toggle (button is now always visible). This is consistent with, not a reversal of, the existing "console stays dark on purpose" decision — it closes a gap in that pin's coverage rather than reopening the dark-vs-light question. Real light mode for the console's content (the ~418-instance conversion) is scoped as its own follow-up pass under this milestone, not silently folded into this fix, given the regression risk of rushing it. `npm run build` and `npm test` clean (135 passed, 6 skipped, 0 regressions).

**Risks (added):**
- Treating "real light mode" as a drop-in toggle flip risks shipping unreadable white-on-white content given how extensively the console's content is hardcoded for dark — Mitigation: scope the conversion as its own reviewed pass (ink token + per-section audit), not a single blind find-replace

**Follow-up pass executed (2026-06-18, same thread): real light mode for the console's content.** Product owner noticed the toggle now switched icons but the console content stayed visually identical between themes ("vc consegue ver a diferença... parece piada") — an accurate read: the prior fix only repaired the acute invisibility bug, it did not deliver the light-mode content conversion scoped above as a follow-up. Executed that follow-up: added `--color-ink` (Tailwind `@theme`) backed by `--ink-rgb` (near-white in dark, near-navy in light, mirroring `--text-primary`), so `text-ink/NN`, `border-ink/NN`, `bg-ink/NN` etc. keep working with opacity modifiers exactly like the `white` utilities they replace. Converted 483 `text-white`/`border-white`/`bg-white`/`divide-white`/`from-white` occurrences (slash-opacity and bare forms) across the console's render tree to their `ink` equivalents via a scripted pass, explicitly excluding: (a) any `print:`-prefixed variant (unrelated to the toggle, paper export stays black-on-white), (b) 29 lines pairing literal `bg-white`/`text-black`/`bg-emerald-500 text-black` etc. — intentional solid-color CTA chips, selected-state pills, and the Nova Consulta/Forge-Monitor invert-buttons, which must stay literal white/black regardless of theme by design, not become theme-relative. The console root's inline style pin (the dark-only override from the prior fix) was replaced with a `theme === 'light' ? {...} : {...}` branch read from `useTheme()`: both branches still re-tint `--glass-fill/-2` and the mesh palette to prevent the blob-bleed bug found earlier in this milestone, just tuned per theme, instead of forcing dark always. `--bg-*`/`--text-*`/`--border` are no longer pinned at all — they now flow from the same global `[data-theme]` tokens the rest of the app uses, which is what actually fixes the toggle's root cause (background and text are driven by the same theme signal, not independently pinned).

Known gap, disclosed rather than silently left: a handful of structural step-indicator elements (the four simulation-progress circles, ~`App.tsx` lines 4009-4039) keep a literal `border-white`/`bg-[#0F1012]` fallback for their inactive state — low-traffic decorative icons, not part of the panels the product owner has been screenshotting, left out of this pass's scope intentionally. Could not render a live screenshot in this environment (no browser/Playwright tooling available in this container) to visually confirm before push — `npm run build` and `npm test` clean (135 passed, 6 skipped, 0 regressions) is the verification done before this push; visual confirmation depends on the product owner's next staging screenshot, same as every prior round in this thread.
