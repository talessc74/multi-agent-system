...
**Revisor:** Revisor do Sistema (Kern: SEED_SYS_001)  
...
| Rastreabilidade agente → seed | 🟡 AMARELO | 1 seed (SEED_JUR_001) compartilhada por 2 agentes — válido, documentado |
...
| agente_claude_code_expert_v1.0 | SEED_ENG_001_v1.0 | IA & Tech | ⚠️ Legacy (ver §4) |
...
| juiza_rosemarie_v1.0 | SEED_JUR_003_v2.0 | Jurídico Consumerista | ✅ Sim |
| advogado_mannrich_v1.0 | SEED_JUR_004_v1.0 | Jurídico Trabalhista | ✅ Sim |
| revisor_sistema_v1.0 | SEED_SYS_001_v1.0 | IA & Tech (sistema) | ✅ Sim |
...
**Problema:** `total_sementes: 7` no SEEDS_REGISTRY enquanto o arquivo listava 8 seeds. O erro foi introduzido na v3.0.0 — a seed SEED_JUR_003_v2.0 foi adicionada à lista mas o counter não foi incrementado.

**Ação:** Corrigido nesta revisão. Counter atualizado para 9 (8 preexistentes + SEED_SYS_001).
...
**Problema:** O agente mais antigo do sistema usava campos não-padrão em seu bloco `versao`:
- `"kernel": "SEED_ENG_001_LEGACY_KERNEL"` em vez de `"kernel": "SHAW_AUDITOR_KERN_0XF1"`
- `"semente_origem": "SEED_ENG_001_v1.0.json"` em vez de `"seed_utilizada": "SEED_ENG_001_v1.0"`
- Campo `gerado_por` ausente no bloco `versao`

O agente foi criado antes da padronização dos campos. Funcionalmente está correto — a seed SEED_ENG_001_v1.0 existe e o agente opera dentro do padrão de blocos (`logicaArquivos`, `logicaInterpretacao`, etc).
...
| `advogado_consumerista_v1.0` | `juiza_rosemarie_v1.0` | Mesma matéria (trabalhista/consumerista) | ✅ VÁLIDA — advogado é operacional/estratégico (1ª instância); SEED_JUR_003 é jurisprudencial/TRT-9 (recursal) |
| `juiz_jec_v1.0` | `juiza_rosemarie_v1.0` | Mesma matéria, instâncias diferentes | ✅ VÁLIDA — JEC é 1ª instância/conciliação; TRT-9 é uniformização jurisprudencial |
...
| `juiz_everton` + `juiza_rosemarie` | Análise em dois graus: decisão TJPR + posicionamento provável no TRT-9 |
| `advogado_consumerista` + `juiza_rosemarie` | Estratégia de recurso: o que peticionar para maximizar chances no TRT-9 |
